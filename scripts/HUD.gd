...
func update_tile_display(i, j):
	...
	if tile.get_zone() != Tile.TileZone.NONE:
		$BottomBar/HoverText.text += "     Zone: %s, Buildings: %s / %s, People: %s / %s, Tile Durability: %s%%, Zone Desirability: %s%%, Tile Value: %s" % [zone_values[tile.get_zone()], tile.data[0], tile.data[1], tile.data[2], tile.data[3], tile.tileDamage, tile.desirability, tile.landValue]
		if tile.powered:
