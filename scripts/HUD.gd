...
func update_tile_display(i, j):
	...
	if tile.get_zone() != Tile.TileZone.NONE:
		$BottomBar/HoverText.text += "     Zone: %s, Buildings: %s / %s, People: %s / %s, Tile Damage: %s%%, Happiness: %s%%, Tile Value: %s" % [zone_values[tile.get_zone()], tile.data[0], tile.data[1], tile.data[2], tile.data[3], tile.tileDamage, tile.happiness, tile.landValue]
		if tile.powered:
